...
public class MessageCalculatorTest {

    @Test
    public void testMakeMessage() throws Exception {
        List<String> input = Arrays.asList("position=< 9,  1> velocity=< 0,  2>", "position=< 7,  0> velocity=<-1,  0>", "position=< 3, -2> velocity=<-1,  1>",
        ...
        MessageCalculator calculator = new MessageCalculator(input);
        Assert.assertEquals(3, calculator.makeMessage());
    }
