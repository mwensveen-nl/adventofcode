...
public class SETITest {
    ...
    public void testPerform() throws Exception {
        OpCode opCode = new SETI();
        List<Integer> regIn = Arrays.asList(1, 3, 4, 8);
        List<Integer> inst = Arrays.asList(1, 2, 3, 3);
        Assert.assertEquals(Arrays.asList(1, 3, 4, 2), opCode.perform(regIn, inst));
        inst = Arrays.asList(1, 4, 1, 0);
        Assert.assertEquals(Arrays.asList(4, 3, 4, 8), opCode.perform(regIn, inst));
    }
