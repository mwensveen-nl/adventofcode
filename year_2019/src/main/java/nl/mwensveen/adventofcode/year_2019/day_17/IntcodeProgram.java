...
public class IntcodeProgram {
    ...
    private int inputValuesIndex = 0;
    ...
    public List<BigInteger> process(int input, List<Integer> inputs) {
        inputValues = inputs;
    ...
    private void input(BigInteger index, Instruction inst) {
        ...
        BigInteger in = getInputValue();
        program.put(value1, in);
        // System.err.println("Input pos " + value1 + " -> " + inputValue);
    }

    private BigInteger getInputValue() {
        if (inputValues == null) {
            return inputValue;
        }
        BigInteger in = BigInteger.valueOf(inputValues.get(inputValuesIndex));
        inputValuesIndex++;
        return in;
    }
