...
public class CardsWinningCalculatorTest {
	...
	public void testWinnings() {
		...
		List<Card> cards = cardparser.parse(input.lines().toList());
		CardsWinningCalculator calculator = new CardsWinningCalculator();
		Long winnings = calculator.winnings(cards);
		assertEquals(13, winnings);
	}

	@Test
	public void testScratchcardsWinnings() {
		Cardparser cardparser = new Cardparser();
		List<Card> cards = cardparser.parse(input.lines().toList());
		CardsWinningCalculator calculator = new CardsWinningCalculator();
